...
const LoginDetails = () => {
  const {
    ...
    formState: { errors },
    setError,
  } = useForm<RegisterLoginDTO>({ resolver: yupResolver(schema) });

  const [step, setStep] = useRecoilState(RegisterAtom);
  const [_, setUser] = useRecoilState(RegisterUserAtom);

  const onSubmit = handleSubmit(async (login) => {
    const duplicateUser = await getUserByEmail(login.email);
